...
The `max_replication_slots` also sets the maximum number of replication origins,
and some of the functionality of parallel apply uses extra origin per writer.
Hence with parallel apply turned ON, the `max_replication_slots` must be set to
`N \* (number of writers)` plus the number of slots given by the above formula. 
